...
public class FakeFamilyRepository(FamilyEntity? existingFamilyForOwner = null) : IFamilyRepository
{
    ...
    public Task<FamilyEntity?> GetByIdAsync(FamilyId id, CancellationToken cancellationToken = default) =>
        Task.FromResult(_seeded.FirstOrDefault(f => f.Id == id));

    public Task<FamilyEntity?> GetByIdWithMembersAsync(FamilyId id, CancellationToken cancellationToken = default) =>
        Task.FromResult(_seeded.FirstOrDefault(f => f.Id == id));

    public Task<FamilyEntity?> GetByOwnerIdAsync(UserId ownerId, CancellationToken cancellationToken = default) =>
        Task.FromResult(existingFamilyForOwner ?? _seeded.FirstOrDefault(f => f.OwnerId == ownerId));

    public Task<bool> ExistsByIdAsync(FamilyId id, CancellationToken cancellationToken = default) =>
        Task.FromResult(_seeded.Concat(AddedFamilies).Any(f => f.Id == id));

    public Task<bool> UserHasFamilyAsync(UserId userId, CancellationToken cancellationToken = default) =>
        Task.FromResult(existingFamilyForOwner is not null);

    public Task AddAsync(FamilyEntity family, CancellationToken cancellationToken = default)
    {

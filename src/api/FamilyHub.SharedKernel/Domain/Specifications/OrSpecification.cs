...
/// Either specification must be satisfied for the entity to match.
/// </summary>
/// <typeparam name="T">The entity type being evaluated.</typeparam>
/// <param name="left">The left specification.</param>
/// <param name="right">The right specification.</param>
public sealed class OrSpecification<T>(IQueryableSpecification<T> left, IQueryableSpecification<T> right)
    : Specification<T>
    where T : class
{
    /// <inheritdoc/>
    ...
    public override async Task<bool> IsSatisfiedByAsync(T entity, CancellationToken cancellationToken = default)
    {
        ...
        if (leftResult)
        {
            return true; // Short-circuit: no need to evaluate right if left is true
        }
